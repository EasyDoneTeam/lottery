...
import "./interfaces/IMigrationAgent721.sol";
import "./owner.sol";
...
contract ERC721 is Ownable
{
  ...
  //function fillLotteryTicket(uint256 tokenId, uint8 digit1, uint8 digit2, uint8 digit3, bytes32 encodedCombination) public
  ...
  function getTokenData(uint256 _token_id) public view returns (address tokenOwner, bytes32 encodedDigit1, bytes32 encodedDigit2, bytes32 encodedDigit3, bytes32 secretPhraseHash, uint lotteryDeadline) 
  {
      return (tokens[_token_id].tokenOwner, tokens[_token_id].encodedDigit1, tokens[_token_id].encodedDigit2, tokens[_token_id].encodedDigit3, tokens[_token_id].secretPhraseHash, tokens[_token_id].lotteryDeadline);
  }
  ...
  function migrate() external {
        ...
        require(migrated_clients[msg.sender] == 0, "Client migrated already");

        // search by laps
        uint client_tickets = 0;
        uint256 current_lap = lottery.getCurrentLotteryId();
        for(uint lap=1; lap<=current_lap; lap++){
          // get client's tokens
          uint256[] storage tickets = lotteryUsersTickets[lap][msg.sender];
          for(uint i=0; i<tickets.length; i++){
            uint ticket_num = tickets[i];
            lotteryTicket storage _token = tokens[ticket_num];
            bytes32[] memory digs = new bytes32[](3);
            digs[0] = _token.encodedDigit1;
            digs[1] = _token.encodedDigit2;
            digs[2] = _token.encodedDigit3;
            //bytes32[] memory digs = new bytes32[](_token.encodedDigit1, _token.encodedDigit2, _token.encodedDigit3);
            bytes32 _secret = _token.secretPhraseHash;
            uint _deadline = _token.lotteryDeadline;
            IMigrationAgent721(migrationAgent).migrateToken(msg.sender, digs, _secret, lap, _deadline, ticket_num);
            totalMigrated += 1;
            client_tickets += 1;
            delete tokens[ticket_num];
          }
          //tickets = new uint256[](0);
        }
        migrated_clients[msg.sender] = client_tickets;

        emit Migrate(msg.sender, migrationAgent, client_tickets);
  }
